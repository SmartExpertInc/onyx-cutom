...
// Test new ClickableImagePlaceholder implementation
export const testNewImagePlaceholder = () => {
  if (typeof window === 'undefined') return;
  
  console.group('[Debug] New ClickableImagePlaceholder Test');
  ...
  imagePlaceholders.forEach((el, index) => {
    ...
    console.log(`Image placeholder ${index + 1}:`, {
      ...
      hasPlaceholder: !!el.querySelector('.text-gray-500')
    });
  });
  ...
  if (imagePlaceholders.length > 0) {
    ...
    const hasImage = !!firstPlaceholder.querySelector('img');
    console.log('Has image:', hasImage);
    
    if (hasImage) {
      // If it has an image, test moveable functionality
      console.log('Testing moveable functionality on image...');
      firstPlaceholder.click();
      
      setTimeout(() => {
        const controls = document.querySelectorAll('.moveable-control');
        console.log('Moveable controls found:', controls.length);
        
        if (controls.length === 0) {
          console.warn('No moveable controls found - possible issue with Moveable component');
        } else {
          console.log('Moveable controls found - system working correctly');
        }
      }, 200);
    } else {
      // If no image, test upload functionality
      ...
      setTimeout(() => {
        ...
        if (modals.length === 0) {
          console.warn('No upload modal found - possible issue with upload functionality');
        } else {
          console.log('Upload modal found - system working correctly');
        }
      }, 200);
