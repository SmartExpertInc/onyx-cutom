...
interface ProjectsTableProps {
  ...
  showFolderButtonInSelectionMode?: boolean;
}
              ...
              <TableCell className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {p.designMicroproductType ? (
                  <span className="text-gray-500 font-medium">
                    {getProductTypeDisplayName(p.designMicroproductType, t)}
                  </span>
                ) : (
                  "-"
                )}
...
const MyProductsTable: React.FC<ProjectsTableProps> = ({
  trashMode = false,
  folderId = null,
  auditMode = false,
  ...
  onToolbarRender,
  showFolderButtonInSelectionMode = false,
}) => {
  ...
  const handleRenameFolderClick = (folder: Folder, e?: React.MouseEvent) => {
    if (e) {
      e.stopPropagation();
    }
  ...
  const handleRenameFolderSubmit = async () => {
    ...
    try {
      ...
      // Try PATCH method first, as it's more appropriate for partial updates
      let response = await fetch(`${CUSTOM_BACKEND_URL}/projects/folders/${folderToRename.id}`, {
        method: 'PATCH',
        headers,
        credentials: 'same-origin',
      ...
      // If PATCH with folder_name fails, try PATCH with 'name' field
      if (!response.ok && response.status !== 401 && response.status !== 403) {
        console.log('First attempt failed, trying with name field...');
        response = await fetch(`${CUSTOM_BACKEND_URL}/projects/folders/${folderToRename.id}`, {
          method: 'PATCH',
          headers,
          credentials: 'same-origin',
      ...
      // If PATCH method doesn't work, try POST to rename endpoint
      if (!response.ok && response.status !== 401 && response.status !== 403) {
        console.log('PATCH method failed, trying POST to rename endpoint...');
        response = await fetch(`${CUSTOM_BACKEND_URL}/projects/folders/${folderToRename.id}/rename`, {
          method: 'POST',
          headers,
          credentials: 'same-origin',
          body: JSON.stringify({ name: newFolderName.trim() })
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to rename folder: ${response.status} ${response.statusText}. ${errorText}`);
      }
  ...
  // Content type filters for list view - use constant keys
  const contentTypeFilterKeys = ["All", "Course", "Presentation", "Quiz", "Video Lesson", "One-Pager"];
  
  const contentTypeFilterLabels: Record<string, string> = {
    "All": t("interface.all", "All"),
    "Course": t("interface.course", "Course"),
    "Presentation": t("interface.presentation", "Presentation"),
    "Quiz": t("interface.quiz", "Quiz"),
    "Video Lesson": t("interface.videoLesson", "Video Lesson"),
    "One-Pager": t("interface.onePager", "One-Pager"),
  };
  ...
  // Define types for combined rows
  type CombinedRow = 
    | { type: 'back'; folder: null }
    | { type: 'folder'; folder: Folder }
    | { type: 'project'; project: Project };

  // Create a combined list of folders and projects for pagination in list mode
  const getCombinedRowsForList = (): { allRows: CombinedRow[]; folders: Folder[]; projects: Project[] } => {
    if (viewMode !== "list") return { allRows: [], folders: [], projects: [] };
    
    if (selectedFolderId) {
      // Inside a folder: show back row + projects only
      return {
        allRows: [
          { type: 'back', folder: null } as const,
          ...visibleProjects.map(p => ({ type: 'project', project: p } as const))
        ],
        folders: [],
        projects: visibleProjects
      };
    } else {
      // Main view: show folders + projects
      return {
        allRows: [
          ...folders.map(f => ({ type: 'folder', folder: f } as const)),
          ...visibleProjects.map(p => ({ type: 'project', project: p } as const))
        ],
        folders: folders,
        projects: visibleProjects
  ...
  const { allRows, folders: visibleFolders, projects: allProjects } = getCombinedRowsForList();

  // Pagination logic - now includes folders
  ...
  const navigationPanel = useMemo(() => {
    ...
    const alignmentClasses = isExternalToolbar
      ? (showFolderButton ? 'justify-between' : 'justify-end')
      : (showFolderButton ? 'justify-between mb-12' : 'justify-end mb-12');

    return (
      <div className={`flex gap-4 items-center w-full ${alignmentClasses}`}>
        {showFolderButton && (
          <div className="flex">
            <Button 
    ...
    <div>
      ...
      {!trashMode && folders.length > 0 && viewMode === "grid" && (
        <div className="mb-8">
          ...
          {selectedFolderId && (
            <div className="mb-4">
              <button
                ...
                className="px-4 cursor-pointer py-2 rounded-lg text-sm font-medium transition-all bg-gradient-to-b from-gray-50 to-gray-100 text-gray-700 hover:bg-gray-300"
              >
                {`< Back`}
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-9 gap-y-5">
            {folders.map((folder) => (
              <div
                key={folder.id}
                className={`bg-white border rounded-lg p-2 shadow-sm hover:shadow-md transition-all cursor-pointer ${
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-regular text-xs text-gray-900 truncate">{folder.name}</h3>
                  ...
                  <div className="flex items-center">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={(e) => handleRenameFolderClick(folder, e)}>
                          <PenLine size={16} className="text-gray-500" />
                          <span>Rename</span>
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={(e) => handleDeleteFolderClick(folder, e)}>
          ...
          >
            ...
            >
              <TableHeader className="bg-white">
                <TableRow>
                    <TableHead
                      className="px-3 py-2 text-left text-xs font-normal text-gray-500 tracking-wider relative"
                    ...
                    <TableHead
                      className="px-3 py-2 text-left text-xs font-normal text-gray-500 tracking-wider relative cursor-pointer hover:bg-gray-50"
                  ...
                  <TableHead
                    className="px-3 py-2 text-right text-xs text-uppercase font-normal text-gray-500 tracking-wider"
                    style={{ width: "20px" }}
                  >
              ...
              <TableBody className="bg-white divide-y divide-[#E0E0E0]">
                {/* Render paginated rows (folders and projects) */}
                {paginatedRows.map((row, index) => {
                  ...
                  if (row.type === 'folder') {
                    const folder = (row as { type: 'folder'; folder: Folder }).folder;
                    return (
                      <TableRow
                        key={`folder-${folder.id}`}
                      ...
                      >
                        <TableCell className="px-3 py-3 whitespace-nowrap">
                          <div className="flex items-center gap-2">
                              <svg width="25" height="25" fill="none" viewBox="0 0 24 24" className="text-[#71717A]">
                                <path d="M3 7a2 2 0 0 1 2-2h3.172a2 2 0 0 1 1.414.586l.828.828A2 2 0 0 0 12.828 7H19a2 2 0 0 1 2 2v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7z" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" />
                              </svg>
                            <span className="text-xs font-medium text-[#71717A]">Folder</span>
                          </div>
                        </TableCell>
      ...
      {/* Rename Folder Modal */}
      {renameFolderModalOpen && folderToRename && (
