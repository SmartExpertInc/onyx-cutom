...
export default function AnimateButton({ isVisible, position, onClick }: AnimateButtonProps) {
  ...
  const button = (
    <button
      ...
      className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-[#4D4D4D] bg-white border border-[#E0E0E0] rounded-md shadow-sm hover:bg-[#F7F7F8] transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#4D4D4D]/40"
    >
      <svg
        width="16"
        height="16"
