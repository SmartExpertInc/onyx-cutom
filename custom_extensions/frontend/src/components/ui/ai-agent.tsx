...
export const AiAgent: React.FC<AiAgentProps> = ({
  ...
  return (
    <div 
      ...
      className="bg-white border border-[#E0E0E0] rounded-lg py-8 px-10 flex flex-col gap-6 mt-3" 
    ...
    >
        ...
        {examples.map((ex) => {
          ...
          return (
            <button
              ...
              style={
                isSelected
                  ? { backgroundColor: '#F7E0FC', color: '#5D5D79', borderColor: '#5D5D79' }
                  : { color: '#5D5D79', borderColor: '#5D5D79' }
              }
            >
              <SparkleIcon color="#5D5D79" />
              <span>{ex.short}</span>
            </button>
          );
        })}
      ...
      <div className="relative">
        <Textarea
          ...
          className="w-full px-5 py-4 pb-14 rounded-lg bg-white text-sm text-black resize-none overflow-hidden min-h-[120px] border-[#E0E0E0] focus:border-[#8808A2] focus:outline-none focus:ring-0 transition-all duration-200 placeholder:text-sm hover:shadow-lg cursor-pointer"
