...
interface BaseGoogleCredentialFormProps {
  connectorId: 'google_drive' | 'gmail';
...
const BaseGoogleCredentialForm: FC<BaseGoogleCredentialFormProps> = ({
  connectorId,
  connectorName,
  authCookieName,
  onCredentialCreated,
  ...
  const refreshCredentialCheck = async () => {
    try {
      ...
      const credentialsResponse = await fetch(`/api/custom-projects-backend/credentials/${connectorId}`);
  ...
  const handleDeleteCredentials = async (credentialId?: number) => {
    ...
    try {
      ...
      if (credentialId) {
        ...
        response = await fetch(`/api/custom-projects-backend/credentials/${credentialId}`, {
          method: "DELETE",
        });
      } else {
  ...
  const handleOAuthAuthentication = async () => {
    ...
    try {
      ...
      document.cookie = `${authCookieName}=true; path=/`;
      ...
      setError(`Failed to authenticate with ${connectorName} - ${err}`);
          ...
          <div className="flex items-start">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-green-700 mb-3">
                Your {connectorName} credentials have been successfully uploaded and authenticated.
              </p>
            </div>
          </div>
        </div>
    ...
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-2">
          {connectorName} Credential Setup
        </h3>
        <p className="text-sm text-gray-600 mb-4">
          To connect your {connectorName}, create credentials (OAuth App only), 
        ...
        <div>
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
            <p className="text-sm text-blue-800">
              Next, you need to authenticate with {connectorName} via OAuth. This gives us read access 
              to the {connectorId === 'gmail' ? 'emails' : 'documents'} you have access to in your {connectorName} account.
            </p>
          </div>
          <Button
          ...
          >
            {isAuthenticating ? "Authenticating..." : `Authenticate with ${connectorName}`}
          </Button>
        </div>
      )}
...
export default BaseGoogleCredentialForm;
