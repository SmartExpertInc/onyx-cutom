...
const ExplanationField: React.FC<ExplanationFieldProps> = ({
  ...
  return (
    <div className={`${className} p-2 md:p-4 bg-[#D8FDF9] rounded-lg`}>
...
const QuizDisplay: React.FC<QuizDisplayProps> = ({ dataToDisplay, isEditing, onTextChange, parentProjectName, lessonNumber, isAuthorized = true, onAuthorizationChange, onExport }) => {
  ...
  const renderMatching = (question: MatchingQuestion, index: number) => {
    const userAnswer = userAnswers[index] || {};
    const canEditMatching = Boolean(isEditing && onTextChange);
          ...
          <div className="p-0">
            {question.prompts.map((prompt, promptIndex) => {
              ...
              return (
                <div
                  key={prompt.id}
                  className="py-0 flex flex-col gap-0 md:gap-3 sm:flex-row sm:items-start"
                >
