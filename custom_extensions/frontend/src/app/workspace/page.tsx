...
import { Home, Users, HardDrive, FileText, LayoutTemplate, Palette, Trash2, Bell } from "lucide-react";
...
const Header = () => {
    ...
    return (
        <header className="flex items-center justify-between p-4 px-8 border-b border-gray-200 bg-white sticky top-0 z-10">
            <h1 className="text-3xl font-bold text-gray-900">{t('interface.workspace', 'Workspace')}</h1>
    ...
    return (
        <aside className="w-64 bg-white p-4 flex flex-col fixed h-full border-r border-gray-200 text-sm">
            ...
            <nav className="flex flex-col gap-1">
                <Link href="/projects" className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-100 text-gray-600">
                    <Home size={18} />
                    <span>{t('interface.products', 'Products')}</span>
                </Link>
                <Link href="/workspace" className="flex items-center gap-3 p-2 rounded-lg bg-blue-50 text-blue-700 font-semibold">
                    <Users size={18} />
                    <span>{t('interface.workspace', 'Workspace')}</span>
                </Link>
                <Link href="/projects?tab=smart-drive" className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-100 text-gray-600">
                    <HardDrive size={18} />
                    <span>{t('interface.smartDrive', 'Smart Drive')}</span>
                </Link>
                <Link href="/projects?tab=offers" className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-100 text-gray-600">
                    <FileText size={18} />
