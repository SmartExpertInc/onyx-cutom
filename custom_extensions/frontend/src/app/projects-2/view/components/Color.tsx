...
interface ColorProps {
  ...
  selectedTarget?: string; // Description of what's being colored (e.g., "Top Section", "Slide Background")
}

export default function Color({ currentBackgroundColor, onColorSelect, selectedTarget }: ColorProps) {
  const [selectedColor, setSelectedColor] = useState<string | null>(currentBackgroundColor || null);
      ...
      {selectedColor && (
        <div className="absolute top-0 left-0 right-0 z-20 bg-gray-100 p-4 flex items-center justify-between">
          <div className="flex flex-col space-y-1">
            ...
            {selectedTarget && (
              <span className="text-xs ml-11" style={{ color: '#999' }}>
                Selected color will be applied
              </span>
            )}
          </div>
          <button 
